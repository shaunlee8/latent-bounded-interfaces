...
from interfaces import LegacyVectorMLPInterfaceView
...
class NativeRegionInterfaceModel(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.interface = LegacyVectorMLPInterfaceView(
            interface_width=message_dim,
            initial_encoder=self.input_to_message,
            decoders=self.message_to_hidden,
            encoders=self.hidden_to_message,
            norms=self.message_norm,
        )
    ...
    def _pool_hidden(self, x: torch.Tensor) -> torch.Tensor:
        """Compatibility helper retained while cache/pullback code uses LBI-1 names."""
        return self.interface.pool_hidden(x)
    ...
    def _forward_region_message_map(
        ...
        hidden_bias = self.interface.decode(m_in, ridx)
        x_in = x_static + hidden_bias.unsqueeze(1)
        x_out = self.backbone.forward_range(x_in, start, end)
        return self.interface.update(x_out, m_in, ridx, self.message_alpha[ridx]).state
    ...
    def forward_with_cache(
        ...
        *,
        ...
        m = self.interface.initial_state(x_static)
        ...
        for ridx, (start, end) in enumerate(self.region_ranges):
            ...
            interface_update = self.interface.update(x_out, m_in, ridx, self.message_alpha[ridx])
            pooled_hidden = interface_update.pooled_region_output
            delta_m = interface_update.delta_state
            alpha = interface_update.update_scale
            pre_norm_message = interface_update.pre_norm_state
            m = interface_update.state
            m = self._apply_message_ablation(
                m,
                mode=message_ablation,
